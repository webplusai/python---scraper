...
class ForumsSpider(CrawlSpider):
    ...
    def parsePostsList(self,response):
        ...
        for post in posts:
            ...
            item['create_date'] = self.getDate(self.cleanText(" ".join(post.xpath('.//span[@class="post-created hidden-xs"]/text()').extract_first())))
            if not item['create_date']:
                item['create_date'] = self.getDate(self.cleanText(" ".join(post.xpath('.//span[@class="post-is-reply-to"]/text()').extract_first().replace('in response to','').strip())))
