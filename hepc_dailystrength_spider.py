...
class ForumsSpider(CrawlSpider):
    ...
    def parsePostsList(self,response):
        ...
        create_date = self.getDate(self.cleanText(" ".join(response.xpath('.//div[contains(@class, "discussion_text")]/span/text()').extract_first().replace(u'Posted on','').strip())))
        ...
        for post in posts:
            ...
            if item['author']:
                ...
                create_date = self.getDate(self.cleanText(" ".join(post.xpath('.//span[@class="graytext"][2]/text()').extract_first().strip())))
                item['create_date'] = create_date
