...
class ForumsSpider(CrawlSpider):
    ...
    def parsePostsList(self,response):
        ...
        for post in posts:
            ...
            item['create_date'] = self.getDate(self.cleanText(" ".join(post.xpath('.//span[@class="vt_first_timestamp"]/text()').extract_first())))
            if not item['create_date']:
                item['create_date'] = self.getDate(self.cleanText(" ".join(post.xpath('.//div[@class="vt_reply_timestamp"]/text()').extract_first().replace('replied','').strip())))
